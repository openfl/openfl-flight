...
Final reconciliation was performed after rebasing onto `fd42882`, with this
assignment's TileContainer, Lib, and shader cleanup applied. The requested
canonical sweep, `grep -rn "// TODO" src/openfl/ --include="*.hx"`, finds 83
entries. A broader `TODO` sweep finds three additional spellings, included here
so the inventory covers all 86 remaining markers:
...
This assignment removed five TileContainer allocation markers, two resolved
Lib markers, and five shader-model markers. The shader limitations remain
documented in source and in `agents/flight-gaps.md`, without being left as
unclassified TODOs.
...
| --- | ---: |
| implemented | 1 |
| blocked:GL-draw-seam | 29 |
| blocked:binding-regen | 0 |
| blocked:flight-gap | 18 |
| adapter-todo | 22 |
| optimization | 4 |
| intentional-stub | 12 |
| **Total** | **86** |
...
| Location | Count | Inventory |
| --- | ---: | --- |
| `src/openfl/display3D/Context3D.hx:59,67,107,112,117,122,127,132,137,142,147,153,158,163,168,173,178,183,190,195,200,205` | 22 | Render-target setup, resource lifecycle/readback, drawing, presentation, GPU state, program/constants, render targets, samplers, scissor/stencil, textures, and vertex buffers need a public immediate-mode draw seam. |
...
| `src/openfl/display/BitmapData.hx:491,518` | 2 | Index- and vertex-buffer cache exposure requires the Stage3D geometry bridge recorded under **BitmapData platform and Stage3D cache types**. |
...
| `src/openfl/display/BitmapData.hx:425,432,452,459` | 4 | **Synchronous BitmapData image construction** — Flight's public image decoding/resource pipeline is asynchronous and has no Lime-image handle bridge. |
| `src/openfl/display/BitmapData.hx:499` | 1 | **BitmapData platform and Stage3D cache types** — Flight raster surfaces are not compatible with Lime's `CairoImageSurface`. |
...
| `src/openfl/filesystem/File.hx:169,306,399,401,1016,1485,2090,2252,2290` | 9 | Wire cache/symlink/usage/charset surfaces where possible; improve copy error handling, relative paths, Windows hidden metadata, application-directory write protection, and application URL schemes. |

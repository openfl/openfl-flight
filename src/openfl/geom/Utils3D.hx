...
class Utils3D
{
	...
	**/
	...
	{
		return projectVectorToOutput(m, v, null);
	}
	...
	**/
	public static function projectVectorToOutput(m:Matrix3D, v:Vector3D, output:Vector3D):Vector3D
	{
		...
		var outputW = output.w;
		m.transformVectorToOutput(v, output);
		output.project();
		output.w = outputW;
		return output;
	}
	...
	**/
	public static function projectVectors(m:Matrix3D, verts:Vector<Float>, projectedVerts:Vector<Float>, uvts:Vector<Float>):Void
	{
		...
		{
			input.setTo(verts[i], verts[i + 1], verts[i + 2]);
			m.transformVectorToOutput(input, transformed);
			var inverseW = 1 / transformed.w;
			projectedVerts.push(transformed.x * inverseW);
			projectedVerts.push(transformed.y * inverseW);
			uvts[i + 2] = inverseW;
			i += 3;
		}
	}
